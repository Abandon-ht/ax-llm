 ...
 * qwen3_asr_debug.cpp
 * ────────────────────────────────────────────────────────────────────────────
 * Qwen3-ASR AX650 解码器调试工具
 *
 * 功能：
 *   1. 加载 LLM 模型（配置与 axllm 主程序一致）
 *   2. 从 npy_dir 读取预计算的 combined_embed_bf16.bin 和 meta.json
 *   3. 调用 LLM::Run(embed) 执行 prefill + greedy decode
 *   4. 打印识别结果
 *
 * 用法：
 *   qwen3_asr_debug <model_dir> <npy_dir> [max_new_tokens]
 *
 * 其中 npy_dir 由 dump_asr_debug_npy.py 生成，包含：
 *   combined_embed_bf16.bin   bfloat16 raw，形状 [S, hidden_size]
 *   meta.json                 包含 S, hidden_size, audio_token_id 等
 *
 * 编译：
 *   在 ax-llm 工程根目录执行 ./build_ax650.sh
 *   生成的二进制在 build_aarch64/install/bin/qwen3_asr_debug
 * ────────────────────────────────────────────────────────────────────────────
 */
...
int main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr,
            "Usage: qwen3_asr_debug <model_dir> <npy_dir> [max_new_tokens]\n"
            "\n"
            "  model_dir      Qwen3-ASR-0.6B axmodel 目录（含 config.json）\n"
            "  npy_dir        由 dump_asr_debug_npy.py 生成的调试数据目录\n"
            "  max_new_tokens 最多生成 token 数（默认 128）\n"
            "\n"
            "npy_dir 需包含：\n"
            "  combined_embed_bf16.bin   bfloat16 embedding，[S, hidden_size]\n"
            "  meta.json                 形状元信息\n"
        );
        return 1;
    }
    ...
    const int S              = meta.value("S", 390);
    const int S0             = meta.value("S0", S);
    const int hidden_size    = meta.value("hidden_size", 1024);
    const int audio_token_id = meta.value("audio_token_id", 151676);
    ...
    // ── 1. 读取 combined_embed_bf16.bin ────────────────────────────────────
    const std::string embed_path = npy_dir + "/combined_embed_bf16.bin";
